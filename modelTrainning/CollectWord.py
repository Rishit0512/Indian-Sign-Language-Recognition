...
# Setup
GESTURE_CLASSES = [
    "bad", "bye", "difficult", "easy", "fear", "food", "good",
    "Indian", "Language", "Namaste", "Strong", "No", "Practice",
    "Thank you", "Understood", "Welcome"
]

SAVE_DIR = "Word_dataset"
AUG_PER_CAPTURE = 20
IMG_SIZE = 224
MAX_IMAGES_PER_CLASS = 200  # Max number of images per class (excluding augmentation)
...
print("Controls:")
print("• 's' to save 15 images for current word")
print("• 'n' next word, 'p' previous word, 'q' to quit")
...
while True:
    ...
    # Detect hands and get combined bounding box
    ...
    # Show current word
    cv2.putText(frame, f"Word: {label}", (10, 40),
    ...
    if key == ord('s') and hand_img is not None:
        ...
        current_count = len(os.listdir(path)) // 2  # Since each capture = 2 files (RGB + Gray)

        if current_count >= MAX_IMAGES_PER_CLASS:
            print(f"⚠️ Max images reached for '{label}' ({MAX_IMAGES_PER_CLASS}). Skipping save.")
            continue
